import { AbstractMesh, Color3, CreateCylinder, CreateSphere, Observable, Observer, Quaternion, Scene, Vector3 } from "@babylonjs/core"
...
export class N3DConnectionInstance{
    ...
    private _color = Color3.White().toColor4(1)
    ...
    private connect(cA: N3DConnectableInstance, cB: N3DConnectableInstance): boolean{
        ...
        this._color = this.cOutput.config.color.toColor4(1)
        ...
        if(![input.config.direction, output.config.direction].includes("bidirectional")){
            this._arrow = CreateCylinder("connection arrow",{
                height: Node3DInstance.CONNECTION_SIZE_MULTIPLIER,
                diameterBottom: .5*Node3DInstance.CONNECTION_SIZE_MULTIPLIER,
                diameterTop: 0,
                tessellation: 6,
            },this.scene)
            SceneManager.getInstance().getShadowGenerator().addShadowCaster(this._arrow, false)
            MeshUtils.setColor(this._arrow, this._color)
        }

        SceneManager.getInstance().getShadowGenerator().addShadowCaster(this._center, false)

        MeshUtils.setColor(this._tube, this._color)
        MeshUtils.setColor(this._center, this._color)
        ...
        function movetube(){
            if(!connection.buildTimeout) connection.buildTimeout = setTimeout(()=>{
                ...
                // Move the arrow
                if(connection._arrow){
                    const arrowCenter = pointB.add(pointC).scaleInPlace(.5)
                    connection._arrow.setAbsolutePosition(arrowCenter)
                    connection._arrow.rotationQuaternion = orientation
                }
    ...
    public pulse(strength: number, tone: number){
        ...
        const color = Color3.FromHSV(tone*360, 1-strength, 1).toColor4(1).multiplyInPlace(this._color)
        MeshUtils.setColor(this._tube, color)
        if(this._center) MeshUtils.setColor(this._center, color)

        this.pulseTimeout = setTimeout(()=>{
            MeshUtils.setColor(this._tube, this._color)
            if(this._center) MeshUtils.setColor(this._center, this._color)
        }, 100)
    }
