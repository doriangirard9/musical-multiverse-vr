...
export class HandMenu {
    ...
    private init() {
        this.baseCube = CreateBox("hand-menu-base-cube", {
            width: 0.05,
            height: 0.005,
            depth: 0.1
        }, this.scene)
        ...
        this.gazeBehavior.onCustomCheck = () => {
            const leftPointer = this.inputs.left.pointer
            const headPointer = this.inputs.head

            const controllerDown = TmpVectors.Vector3[1]
            controllerDown.copyFrom(leftPointer.up).scaleInPlace(-1)

            const cameraForward = TmpVectors.Vector3[2]
            cameraForward.copyFrom(headPointer.forward)

            const dotProduct = Vector3.Dot(controllerDown, cameraForward)

            const alignmentThreshold = 0.7

            console.log(dotProduct, "Gaze alignment check (dot product), threshold:", alignmentThreshold)

            return dotProduct > alignmentThreshold
        }

        this.gazeBehavior.onGazeActivated = () => {
            console.log("Gaze activated - showing menu")
            if (!this.gazeMenu) {
        ...
        this.beforeRenderObs = this.scene.onBeforeRenderObservable.add(() => {
            // Place base cube on the left controller
            const leftPointer = this.inputs.left.pointer
            this.baseCube!.setAbsolutePosition(leftPointer.origin.add(leftPointer.up.scale(0.01)))
            this.baseCube!.rotationQuaternion = B.Quaternion.FromRotationMatrix(leftPointer.matrix.getRotationMatrix())
            ...
            if (this.gazeMenu && this.gazeMenu.menuNode.mesh) {
                ...
                if (mesh.parent !== this.baseCube) {
                    ...
                    this.gazeMenu.menuNode.position.set(0, 0, 0)
                }
            }
        })
