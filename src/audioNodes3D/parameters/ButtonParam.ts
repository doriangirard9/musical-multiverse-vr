...
export class ButtonParam implements IParameter {
    ...
    public async _createButton(): Promise<void> {
        ...
        this._buttonMesh = result.meshes[0] as B.Mesh;
        this._buttonMesh.scaling = new B.Vector3(0.45, 0.45, 0.45);

        this._cylinder = this._buttonMesh.getChildMeshes(false, (node: B.Node): boolean => { return node.name.indexOf("Cylinder") !== -1 })[0];
        ...
        this._button = new GUI.MeshButton3D(this.buttonMesh, "button");
        this._initButtonEvents();

        manager.addControl(this._button);
        this.buttonMesh.parent = this._parentMesh;
    ...
    public get buttonMesh(): B.Mesh { return this._buttonMesh; }
