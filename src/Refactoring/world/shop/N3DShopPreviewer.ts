...
export class N3DShopPreviewer implements N3DShopType {
    ...
    async create(object: N3DShopObject, shop: N3DShop): Promise<() => Promise<void>> {
        ...
        const {type, options, location} = object
        ...
        if(!this.kinds.has(object)){
            ...
            else if(options.category && shopOptions.categories)
                kinds = shopOptions.categories[options.category] ?? []
        ...
        const kind = this.kinds.get(object)
