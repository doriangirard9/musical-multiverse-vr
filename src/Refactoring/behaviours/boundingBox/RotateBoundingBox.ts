...
export class RotateBoundingBox implements B.Behavior<B.AbstractMesh> {
    ...
    public onSqueezeReleased(): void {
        ...
        if (this._observer) {
            SceneManager.getInstance().getScene().onBeforeRenderObservable.remove(this._observer);
            this._observer = null;
        }
    }
    ...
    private _startRotation(controller: B.WebXRInputSource): void {
        if (this._selectedMesh) {
            ...
            this._observer = SceneManager.getInstance().getScene().onBeforeRenderObservable.add(() => {
