...
export class FunctionAPI {
    ...
    emitNote(channel: number, note: number, velocity: number, duration: number, startTime?: number) {
        ...
        this.emitMidiEvent([0x90 | channel, note, velocity], startTime)
        this.emitMidiEvent([0x80 | channel, note, velocity], startTime+duration)
    }
     ...
     * */
    emitMidiEvent(bytes: [number,number,number], eventTime: number) {
        if (bytes.length > 3) {
    ...
    getTickDuration(ticks: number): number {
        return ticks * 1.0 / ((this.#kernel.tempo / 60.0) * PPQN);
    }
    ...
    setCustomNoteList(noteList?: NoteDefinition[]) {
        this.#kernel.setNotelist(noteList)
    }
    ...
    getParams(): Record<string, number> {
        ...
        for (let id of this.#kernel.parameterIds) {
            params[id] = this.#kernel.getParameterState(id)
        }
